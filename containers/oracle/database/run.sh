...
echo "$REGISTRY_SECRET" | docker login container-registry.oracle.com --username "$REGISTRY_USERNAME" --password-stdin
...
for token in "${TOKEN_LIST[@]}"; do
  ...
  echo "Replacing \${$token}, \$$token for ${!token} in $DOCKER_COMPOSE_FILE by using this pattern $replace_this and replace it with $with_this"
